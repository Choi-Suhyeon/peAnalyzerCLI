...
ScnHdList * getScnHeaders(TargetFile * file, size_t num_of_scn, size_t initial_adr) {
...
IIDList * getIIDs(
        ...
        size_t                 num_of_IID,
        size_t                 initial_adr,
        std::list<SizeTPair> * va_ptr2raw_lst) {
...
std::list<SizeTPair> * getLstOfMainInfoOfScn(ScnHdList * scn_hds) {
...
void printScnHeaders(const ScnHdList * kScnHeaders) {
    for (auto & kScnHd : *kScnHeaders) kScnHd.print();
}
...
int main(int argc, char ** argv) {
    ...
    auto idd_lst = getIIDs(tf, idd_info.second, idd_info.first, va_ptr2raw_lst);
